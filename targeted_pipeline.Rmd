...
area.table <- peak_list %>%
  select(Replicate, Precursor.Ion.Name, num_area) %>%
  filter(str_detect(Replicate, regex("Smp|Poo", ignore_case = TRUE)))

SN.table <- peak_list %>%
  filter(str_detect(Replicate, regex("Smp|Poo", ignore_case = TRUE))) %>%
  select(Replicate, Precursor.Ion.Name, num_area, num_background) %>%
  mutate(Signal.to.Noise = (num_area / num_background))

blank.table <- peak_list %>%
    filter(str_detect(Replicate.Name, regex("Blk", ignore_case = TRUE))) %>%
    select(Precursor.Ion.Name, num_area) %>%
    group_by(Precursor.Ion.Name) %>% 
    filter(num_area == max(num_area)) %>%
    rename(Blank.max = num_area) %>%
    unique()
```
...
peak_list_all_flags <- peak_list %>%
  ...
  group_by(Precursor.Ion.Name) %>%
  mutate(Blank.Reference = num_area / Blank.max) %>%
  mutate(IS_present = ifelse(str_detect(Replicate.Name, ","), "Internal Std", "Non IS")) %>%
  mutate(blank.Flag = ifelse(((IS_present != "Internal Std") & ((num_area / Blank.max) < blk.thresh)), 
                             "blank.Flag", 
                             ifelse(((IS_present == "Internal Std") & ((num_area / Blank.max) < blk.thresh)),
                                    "IS.blank.Flag", NA))) %>%
  mutate(area.min.Flag = ifelse((num_area < area.min), "area.min.Flag", NA)) %>%
  mutate(Area.with.QC   = ifelse(is.na(area.min.Flag), num_area, NA)) %>%
  select(Replicate:num_area, Area.with.QC, everything()) %>%
  left_join(SN.table) %>%
...
Int.Stds.data %>% 
  # filter(RunType == "Poo") %>% 
  ggplot() + 
  geom_bar(aes(x = Replicate, y = Area.with.QC, color = RunType), stat = "identity", position = "dodge")  +
...
data.long  <- Data.NoIS %>%
  ...
  select(Replicate, metabolite, Area.with.QC) %>%
  filter(str_detect(Replicate, "Poo")) %>%
  group_by(Replicate) %>%
  rename(poo_area = Area.with.QC) %>% 
  na.omit()

is_areas <- Int.Stds.data %>%
  filter(str_detect(Replicate, "Poo")) %>%
  filter(!str_detect(Mass.Feature, "Inj_vol")) %>%
  select(-c(Precursor.Ion.Name, a, RunType, c, d)) %>%
  group_by(Replicate) %>%
  rename(int_area = Area.with.QC) %>% 
  rename(internal_standard = Mass.Feature) %>% 
  group_by(internal_standard) %>% 
  mutate(avg_is_area = mean(int_area))

bmis_areas <- data.long %>% 
  left_join(is_areas, by = c("Replicate")) %>% 
  mutate(area_per_is = poo_area/int_area*avg_is_area) %>%  
  arrange(area_per_is) %>% 
  group_by(Replicate, metabolite) %>% 
  slice(1) %>% 
  mutate(bmis = internal_standard)

bmis_data <- bmis_areas %>% 
  

# data_long_poo <- Data.NoIS %>%
#   filter(str_detect(Replicate, "Poo")) %>%
#   select(Replicate, Precursor.Ion.Name, num_area) %>%
#   pivot_wider(names_from = Precursor.Ion.Name, values_from = num_area)
# 
# is_areas <- Int.Stds.data %>%
#   filter(str_detect(Replicate, "Poo")) %>%
#   filter(!str_detect(Mass.Feature, "Inj_vol")) %>%
#   select(-c(Precursor.Ion.Name, a, RunType, c, d)) %>%
#   pivot_wider(names_from = Mass.Feature, values_from = Area.with.QC)


# need to calculate mean areas and then calculate the variance (RSD) by QCd area / IntStds area x avgIntStds area

# calculate QCd area / IS area then select smallest value

poo_is_area <- data_long_poo %>% 
