...
Full.data.RF <- Full.stds.data %>%
  ...
  mutate(Replicate.Name=str_remove(Replicate.Name, "_\\d+$"))
...
Full.data.RF.ratios <- temp.RF.ratios %>%
  ...
  group_by(Precursor.Ion.Name) %>% filter(n() >= 0) %>%
  ...
  distinct(Precursor.Ion.Name, Type, RF.mean.per_sampleID) %>%
  pivot_wider(names_from = Type, values_from = RF.mean.per_sampleID) %>%
  mutate(RF.ratio = (Standards_Matrix - Water_Matrix)/Standards_Water) %>%
  select(Precursor.Ion.Name, RF.ratio) %>%
  unique()

# If applicable, supplement data with information from calculated Ingalls QE.RF ratios.
# missing.RF <- setdiff(unique(temp.RF.ratios$Precursor.Ion.Name), Full.data.RF.ratios$Precursor.Ion.Name)
# 
# test.standards <- Ingalls.Standards.Join %>%
#   filter(Precursor.Ion.Name %in% missing.RF) %>%
...
# write.csv(Full.data.RF.ratios, paste("data_intermediate/Skyline_ResponseFactorRatios_", currentDate, ".csv", sep = ""))
...
BMISd.data.filtered <- BMISd.data %>%
  ...
  filter(Precursor.Ion.Name %in% Full.data.RF.ratios$Precursor.Ion.Name) %>%
  left_join(Full.data.RF.ratios) %>%
  left_join(Full.data.RF.dimensions %>% select(Precursor.Ion.Name, RF.max, RF.min) %>% unique(), by = "Precursor.Ion.Name") %>%
...
IS.data <- Full.stds.data %>%
  filter(Precursor.Ion.Name %in% IS.key$FinalBMIS) %>%
  mutate(IS_Area = Area.with.QC,
         FinalBMIS = Precursor.Ion.Name) %>%
  select(IS_Area, FinalBMIS, Replicate.Name) %>%
  left_join(IS.key %>% select(FinalBMIS, Precursor.Ion.Name, Concentration_nM))

matched.IS.compounds <- data.frame(Compounds = c(IS.key[ ,"FinalBMIS"], as.character(IS.key[ ,"Precursor.Ion.Name"])))

IS.sample.data <- QCd.data %>%
  left_join(IS.data %>% select(FinalBMIS, Precursor.Ion.Name, Concentration_nM), by = "Precursor.Ion.Name") %>%
  unique() %>%
  ...
  mutate(testcol1 = ifelse(str_detect(Precursor.Ion.Name, ","), sapply(strsplit(Precursor.Ion.Name, ","), `[`, 1), Precursor.Ion.Name)) %>%
  mutate(Names = ifelse(str_detect(testcol1, "-"), sapply(strsplit(testcol1, "-"), `[`, 2), testcol1)) %>%
  mutate(Pairs = ifelse(!str_detect(Precursor.Ion.Name, ","), Precursor.Ion.Name, paste(Names, "IS", sep = "_"))) %>%
...
All.Info <- Quantitative.data %>%
  ...
  left_join(IS.sample.data %>% select(Sample.Name, Precursor.Ion.Name, umol.in.vial_IS)) %>%
...
All.Info.Quantitative <- All.Info %>%
  ...
  left_join(Full.stds.data %>% select(Precursor.Ion.Name, Empirical_Formula)) %>%
  unique()
