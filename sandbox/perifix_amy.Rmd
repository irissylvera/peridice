...
peak_list_pos <- read_csv('Amy_version_Transition_Results.csv') %>% 
  mutate(`Molecule List Name` = str_replace(`Molecule List Name`,
                                            "HILIC_Pos_Standards", 
                                            "HILIC")) %>% 
  rename(column = `Molecule List Name`) %>% 
  rename(metabolite = Molecule) %>% 
  mutate(treatment = str_extract(`Replicate Name`, "Std|Poo|Blk|C|NPF|NF|N|PF|P|F|T0")) %>% 
  mutate(timepoint = str_extract(`Replicate Name`, "T0|Tfinal")) %>% 
  rename(area = Area) %>% 
  rename(replicate = `Replicate Name`) %>% 
  rename(background = Background) %>% 
  rename(precursor_mz = `Precursor Mz`) %>%
...
metadataframe <- peak_list %>%
  ...
  mutate(filename = str_remove(filename, "230213_")) %>% 
  ...
  mutate(treatment = str_extract(filename, "Std|Poo|Blk|C|NPF|NF|N|PF|P|F|T0")) %>% 
  mutate(timepoint=str_extract(filename, "T0|Tfinal")) %>%
...
peak_data_qc <- peak_list %>% 
  ungroup() %>% 
  distinct() %>% 
  mutate(replicate = str_remove(replicate_name, "230213_")) %>% 
...
blank_table <- peak_list %>%
    filter(str_detect(replicate, regex("Blk", ignore_case = TRUE))) %>%
...
peak_list_all_flags <- peak_list %>%
  ...
  mutate(is_present = ifelse(str_detect(replicate, ","), "Internal Std", "Non IS")) %>%
...
isologs_quant <- isologs %>% 
 ...
 #  mutate(area_with_qc = area_with_qc / pc_um) %>% 
  ...
  mutate(umol_enviro = umol_per_vial*400*1e-6/2*1000) %>% 
  select(replicate, metabolite, area_with_qc, umol_enviro) %>% 
...
quant_data <- peak_list %>% 
  ...
  filter(nM > 0) %>% 
  filter(!str_detect(replicate, "Std")) %>%
  filter(metabolite != "Betonicine") %>%
  filter(metabolite != "Hydroxyisoleucine")

iso <- isologs_quant %>% 
  select(metabolite, nM = nmol_enviro, uM = umol_enviro) 

quant_data <- quant_data %>% 
  full_join(iso) %>% 
  filter(!str_detect(metabolite, "L-Valine"))
