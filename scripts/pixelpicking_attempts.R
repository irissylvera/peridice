...
library(xcms)
library(data.table)
msnexp_filled <- readRDS("xcms_output/msnexp_filled.rds")
...
msdata <- list.files("HILICpos", full.names = TRUE) %>%
  str_subset("Poo") %>%
...
interp_dt <- pbapply::pbmapply(function(mzmed_i, rtmed_i, feat_id_i){
    ...
    msdata$MS1[mz%between%pmppm(mzmed_i)] %>%
      split(.$filename) %>%
...
clean_pcamat <- interp_dt[, .(rt=rank(rt), int), by=c("feature", "filename")] %>%
...
pcaoutput$x[,"PC1"] %>% 
  matrix(nrow=50, ncol=6) %>%
  as.data.frame() %>%
  mutate(rownum=row_number()) %>%
  pivot_longer(-rownum, names_to="rt", values_to="int") %>% 
  mutate(rt=factor(rt, levels=paste0("V", 1:50))) %>%
  ggplot() +
...
pcaoutput$rotation %>%
  ...
  rownames_to_column("feature") %>%
  # left_join(classified_feats) %>%
  plot_ly(x=~PC1, y=~PC2, z=~PC3, mode="markers", type="scatter3d",
          opacity=0.5)
...
ui <- function(){fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      p(" "),
      actionButton("chosen_good", label = HTML("Click here to identify selection<br>as 
...
write.csv(data.frame(feature=chosen_feats), row.names = FALSE,
          file = "chosen_feats.csv")

# Fact-check a single feature ----
row_data <- feature_centers %>% filter(feat_id==sample(chosen_feats, 1))
msdata_gp <- msdata$MS1[mz%between%pmppm(row_data$mzmed, 5)] %>%
